...
def main():
    ...
    data_filepath = args.input                 # path to the input expression data
    output_path = args.output                  # path to the output folder
    gene_sets_path = args.geneSetsPath         # path to the folder where gene sets can be found
    project = args.project                     # identifier for the project
    tissue = args.tissue                       # tissue where the input data is sampled from
    label_name = args.label
    variable_gene_name = args.variable

    prior_name = args.prior                    # gene set file names used to learn the gene set activity score
    features_type = args.features              # features used for the annotator
    alpha = args.alpha
    beta = args.beta
    weight_decay = args.gamma
    tau = args.tau
    z_dim = args.dim                           # dimension for the low-dimensional representation

    batch_size = args.batch
    num_epochs_classifier = args.nanno         # number of epochs to pretrain the annotator
    num_epochs_r = args.nscore                 # number of epochs to train the gene set activity model
    num_epochs_z = args.nauto                  # number of epochs to pretrain the AnnoCluster model
    r_epoch = num_epochs_r - 1
    z_epoch = num_epochs_z - 1
    num_epochs_annocluster = args.ncluster     # number of epochs to train the AnnoCluster model

    z_encoder_layers = args.nzenco             # number of hidden layers for encoder of AnnoCluster
    z_decoder_layers = args.nzdeco             # number of hidden layers for decoder of AnnoCluster
    z_encoder_dim = args.dimzenco              # number of nodes for hidden layers for encoder of AnnoCluster
    z_decoder_dim = args.dimzdeco              # number of nodes for hidden layers for decoder of AnnoCluster
    r_encoder_layers = args.nrenco             # number of hidden layers for the encoder of gene set activity scores
    r_decoder_layers = 1
    r_encoder_dim = args.dimrenco              # number of nodes for hidden layers for encoder of gene set activity
    r_decoder_dim = args.dimrdeco              # number of nodes for hidden layers for decoder of gene set activity
    rnetwork = args.network                    # the encoder for the gene set activity model
   ...
   # prior_name: c5.go.bp.v7.4.symbols.gmt + c2.cp.v7.4.symbols.gmt + TF - DNA
    ...
    N = expression_only.N #First dimension of expressions_only
    G = expression_only.G #Second dimension of expressions_only
